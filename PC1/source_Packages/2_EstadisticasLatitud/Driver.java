...
public class Driver {
	public static void main(String[] args) {
		...
		configuracion_job.setJobName("EstadisticasLatitud");

		// El Mapper emite (Text,DoubleWritable), distinto del output final
		// (Text,Text) del Reducer -> hay que declarar el tipo del map output
		// aparte, si no Hadoop asume que es igual al output final y falla.
		configuracion_job.setMapOutputKeyClass(Text.class);
		configuracion_job.setMapOutputValueClass(DoubleWritable.class);
		...
		configuracion_job.setMapperClass(EstadisticasLatitud.Mapper.class);
		configuracion_job.setReducerClass(EstadisticasLatitud.Reducer.class);
